...
object DependencyGraphSpecTest extends Specification {
  def testNNPOfCollapse = {
    val dgraph = DependencyGraph.stringFormat.read("nsubjpass(born_VBN_5_0, Graham_NNP_0_0); nn(California_NNP_3_0, Southern_NNP_2_0); prep_of(Graham_NNP_0_0, California_NNP_3_0); auxpass(born_VBN_5_0, was_VBD_4_0); prep_in(born_VBN_5_0, Germany_NNP_7_0)").
  ...
  def testCollapseDirected = {
    ...
    val dgraph = DependencyGraph.stringFormat.read(pickled)
  ...
  "serializes ok without extra nodes" in {
    ...
    val dgraph = DependencyGraph.stringFormat.read(pickled)
    DependencyGraph.stringFormat.write(dgraph) must_== pickled
    DependencyGraph.multilineStringFormat.read(DependencyGraph.multilineStringFormat.write(dgraph)) must_== dgraph
  }

  "serializes ok with extra nodes" in {
    val pickled = "(._._5_19); nsubj(ran_VBD_1_2, I_PRP_0_0); prt(ran_VBD_1_2, down_RP_2_6); dobj(ran_VBD_1_2, hill_NN_4_15); det(hill_NN_4_15, the_DT_3_11)"
    val dgraph = DependencyGraph.stringFormat.read(pickled)
    DependencyGraph.stringFormat.write(dgraph) must_== pickled
    DependencyGraph.multilineStringFormat.read(DependencyGraph.multilineStringFormat.write(dgraph)) must_== dgraph
  }
  ...
  "deserializes fails on garbage" in {
    ...
    DependencyGraph.stringFormat.read(pickled) must throwA[DependencyGraph.SerializationException]
  }
  ...
  "deserializes ok" in {
    val pickled = "(over_IN_2_0); nsubj(jumped_VBD_1_0, He_PRP_0_0); det(barrier_NN_4_0, the_DT_3_0); prep_over(jumped_VBD_1_0, barrier_NN_4_0)"
    DependencyGraph.stringFormat.read(pickled) must throwA[DependencyGraph.SerializationException].not
  }
