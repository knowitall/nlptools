...
object DependencyPattern {
  ...
  class Parser extends RegexParsers {
    ...
    def nestedCaptureNodeMatcher(implicit lemmatizer: Stemmer): Parser[CaptureNodeMatcher[TokenDependencyNode]] = "{" ~> """\w+""".r ~ ":" ~ simpleNodeMatcherSeq(lemmatizer) <~ "}" ^^ { case alias ~ ":" ~ seq =>
      seq match {
    ...
    def trivialCaptureEdgeMatcher: Parser[EdgeMatcher[TokenDependencyNode]] = "{" ~> """\w+""".r <~ "}" ^^ { case alias =>
