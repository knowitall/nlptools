...
abstract class Stemmer {
  ...
  def stem(word: String, postag: String): String = stem(word)
  ...
  def stemPostaggedToken[T <: PostaggedToken](token: T) = Lemmatized(token, this.stem(token.string, token.postag))
  ...
  def lemmatizePostaggedToken[T <: PostaggedToken](token: T) = Lemmatized(token, this.lemmatize(token.string, token.postag))
}
